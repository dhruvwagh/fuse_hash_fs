...
static const int SSD_READ_LATENCY_US  = 1000;  // microseconds
static const int SSD_WRITE_LATENCY_US = 5000;  // microseconds
...
static std::vector<size_t> g_queueCapacity;          // each entry = MAX_BYTES_PER_DRIVE
...
static void enqueueRequest(FsRequest&& r, const std::string &uniqueKey) {
...

    {
        ...
        // Enqueue the request and reduce capacity
...
static void ll_init(void *userdata, struct fuse_conn_info *conn) {
    (void)userdata;
    (void)conn;
    // Minimal - remove or comment out if you want zero prints:
    std::cout << "[Info] FUSE init callback\n";
}

int main(int argc, char *argv[]) {
    // You can comment these out if you want zero console output:
