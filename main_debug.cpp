...
// -----------------------------------------------------------------------------
// SSD-like latencies
static const int SSD_READ_LATENCY_US  = 1000;
static const int SSD_WRITE_LATENCY_US = 5000;
...
// -----------------------------------------------------------------------------
// Fixed block sizes and queue capacities
static constexpr size_t BLOCK_SIZE           = 4096;   // 4 KB
static constexpr size_t MAX_BLOCKS_PER_DRIVE = 4;      // e.g., 4 blocks
...
struct SsdModel {
    void readLatency() {
        // Debug:
        std::cout << "[Debug] Simulating read latency (" << SSD_READ_LATENCY_US << " us)\n";
        std::this_thread::sleep_for(std::chrono::microseconds(SSD_READ_LATENCY_US));
    }
    void writeLatency() {
        // Debug:
        std::cout << "[Debug] Simulating write latency (" << SSD_WRITE_LATENCY_US << " us)\n";
...
// -----------------------------------------------------------------------------
// Concurrent queue structures (with new capacity controls)
static std::mutex* g_queueMutex           = nullptr;
static std::condition_variable* g_queueCond = nullptr;        // "not empty" signal
static std::condition_variable* g_queueCondNotFull = nullptr; // "has capacity" signal
...
// New: track how many bytes remain available in each drive's queue
static std::vector<size_t> g_queueCapacity; // each entry = MAX_BYTES_PER_DRIVE initially
...
static fuse_ino_t createInode(bool isDir, mode_t mode) {
    ...
    std::cout << "[Debug] Created inode " << ino << " (isDir=" << isDir << ")\n";
    return ino;
}
...
static void enqueueRequest(FsRequest&& r, const std::string &uniqueKey) {
    ...
    // How many bytes does this request need? (Round up to blocks if it's write/read)
    ...
    // Debug
    std::cout << "[Debug][Enqueue] op=" << (int)r.op
              << " queueIdx=" << queueIdx
...

    {
        ...
        if (g_stopThreads.load()) {
            std::cout << "[Debug][Enqueue] Stop signal received, returning.\n";
            return;
        }

        // Debug
        std::cout << "[Debug][Enqueue] Enough capacity on drive " << queueIdx 
                  << ", capacity before: " << g_queueCapacity[queueIdx] << "\n";

        g_queues[queueIdx].push_back(std::move(r));
        g_queueCapacity[queueIdx] -= bytesNeeded;

        std::cout << "[Debug][Enqueue] capacity after: " << g_queueCapacity[queueIdx] 
                  << " (driveIndex=" << queueIdx << ")\n";
    }

    // Notify worker that queue is not empty
    ...
    while (!g_stopThreads.load()) {
        ...
        for (auto &req : localRequests) {
            ...
            if (bytesUsed > 0) {
                ...
                std::cout << "[Debug][Worker] Freed " << bytesUsed 
                          << " from driveIndex=" << driveIndex 
                          << "; capacity now " << g_queueCapacity[driveIndex] << "\n";
                g_queueCondNotFull[driveIndex].notify_all();
            }
        }
    }
    std::cout << "[Debug][Worker] driveIndex=" << driveIndex << " thread exiting.\n";
}

// -----------------------------------------------------------------------------
// Process functions implementation
static void processLookup(FsRequest &r) {
    std::cout << "[Debug][processLookup] parent=" << r.parent << ", name=" << r.name << "\n";
...
static void processMkdir(FsRequest &r) {
    std::cout << "[Debug][processMkdir] parent=" << r.parent << ", name=" << r.name << "\n";
    InodeLockGuard lock(r.parent);
...
static void processCreate(FsRequest &r) {
    std::cout << "[Debug][processCreate] parent=" << r.parent << ", name=" << r.name << "\n";
    InodeLockGuard lock(r.parent);
...
static void processUnlink(FsRequest &r) {
    std::cout << "[Debug][processUnlink] parent=" << r.parent << ", name=" << r.name << "\n";
    InodeLockGuard lock(r.parent);
...
static void processRmdir(FsRequest &r) {
    std::cout << "[Debug][processRmdir] parent=" << r.parent << ", name=" << r.name << "\n";
    InodeLockGuard lock(r.parent);
...
static void processRename(FsRequest &r) {
    std::cout << "[Debug][processRename] oldParent=" << r.parent 
              << ", name=" << r.name 
...
static void processWrite(FsRequest &r) {
    std::cout << "[Debug][processWrite] ino=" << r.ino 
              << ", offset=" << r.offset 
...
static void processRead(FsRequest &r) {
    std::cout << "[Debug][processRead] ino=" << r.ino 
              << ", offset=" << r.offset 
              << ", size=" << r.size << "\n";
...
static void processGetattr(FsRequest &r) {
    std::cout << "[Debug][processGetattr] ino=" << r.ino << "\n";
...
static void processFlush(FsRequest &r) {
    std::cout << "[Debug][processFlush] ino=" << r.ino << "\n";
    InodeLockGuard lock(r.ino);
    fuse_reply_err(r.req, 0);
}

static void processRelease(FsRequest &r) {
    std::cout << "[Debug][processRelease] ino=" << r.ino << "\n";
    InodeLockGuard lock(r.ino);
    fuse_reply_err(r.req, 0);
}

static void processReaddir(FsRequest &r) {
    std::cout << "[Debug][processReaddir] ino=" << r.ino 
              << ", offset=" << r.offset 
              << ", size=" << r.size << "\n";
    InodeLockGuard lock(r.ino);
...
static void processAccess(FsRequest &r) {
    std::cout << "[Debug][processAccess] ino=" << r.ino << ", mask=" << r.access_mask << "\n";
    InodeLockGuard lock(r.ino);
    fuse_reply_err(r.req, 0);
}

static void processStatfs(FsRequest &r) {
    std::cout << "[Debug][processStatfs]\n";
    struct statvfs st{};
    st.f_bsize   = 4096;
    st.f_frsize  = 4096;
...
// FUSE Callbacks
static void ll_lookup(fuse_req_t req, fuse_ino_t parent, const char *name) {
    std::cout << "[Debug][ll_lookup] parent=" << parent << ", name=" << name << "\n";
...
static void ll_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode) {
    std::cout << "[Debug][ll_mkdir] parent=" << parent << ", name=" << name << "\n";
...
static void ll_create(fuse_req_t req, fuse_ino_t parent, const char *name,
    ...
    std::cout << "[Debug][ll_create] parent=" << parent << ", name=" << name << "\n";
...
static void ll_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
    std::cout << "[Debug][ll_open] ino=" << ino << "\n";
    fi->fh = ino;
...
static void ll_write(fuse_req_t req, fuse_ino_t ino, const char *buf,
                    size_t size, off_t off, struct fuse_file_info *fi) {
    std::cout << "[Debug][ll_write] ino=" << ino 
              << ", size=" << size << ", off=" << off << "\n";
...
static void ll_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
                   struct fuse_file_info *fi) {
    std::cout << "[Debug][ll_read] ino=" << ino 
              << ", size=" << size << ", off=" << off << "\n";
...
static void ll_getattr(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
    std::cout << "[Debug][ll_getattr] ino=" << ino << "\n";
...
static void ll_flush(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
    std::cout << "[Debug][ll_flush] ino=" << ino << "\n";
...
static void ll_release(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
    std::cout << "[Debug][ll_release] ino=" << ino << "\n";
...
static void ll_readdir(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
                      struct fuse_file_info *fi) {
    std::cout << "[Debug][ll_readdir] ino=" << ino 
              << ", size=" << size << ", off=" << off << "\n";
...
static void ll_access(fuse_req_t req, fuse_ino_t ino, int mask) {
    std::cout << "[Debug][ll_access] ino=" << ino 
              << ", mask=" << mask << "\n";
...
static void ll_statfs(fuse_req_t req, fuse_ino_t ino) {
    std::cout << "[Debug][ll_statfs] ino=" << ino << "\n";
...
static void ll_unlink(fuse_req_t req, fuse_ino_t parent, const char *name) {
    std::cout << "[Debug][ll_unlink] parent=" << parent << ", name=" << name << "\n";
...
static void ll_rmdir(fuse_req_t req, fuse_ino_t parent, const char *name) {
    std::cout << "[Debug][ll_rmdir] parent=" << parent << ", name=" << name << "\n";
...
static void ll_rename(fuse_req_t req, fuse_ino_t parent, const char *name,
                      fuse_ino_t newparent, const char *newname, unsigned int flags) {
    std::cout << "[Debug][ll_rename] parent=" << parent << ", name=" << name
...
static void ll_lookup_root(fuse_req_t req, fuse_ino_t parent, const char *name) {
    std::cout << "[Debug][ll_lookup_root] parent=" << parent << ", name=" << name << "\n";
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "[Info] Creating fuse_session\n";
    ...
    std::cout << "[Info] Setting fuse signal handlers\n";
    ...
    std::cout << "[Info] Mounting fuse_session\n";
    ...
    // Initialize root inode
    {
        ...
        std::cout << "[Debug] Root inode (ID=1) created.\n";
    }

    // Initialize concurrency structures
    std::cout << "[Info] Initializing concurrency structures\n";
    g_queueMutex      = new std::mutex[NUM_DRIVES];
    g_queueCond       = new std::condition_variable[NUM_DRIVES];      // not empty
    g_queueCondNotFull= new std::condition_variable[NUM_DRIVES];      // has capacity
    ...
    // Create SSD models
    std::cout << "[Info] Creating " << NUM_DRIVES << " SSD models.\n";
    for (int i=0; i<NUM_DRIVES; i++) {
        g_ssdModels.push_back(std::make_unique<SsdModel>());
    }

    // Start worker threads
    std::cout << "[Info] Spawning " << NUM_DRIVES << " worker threads.\n";
    for (int i=0; i<NUM_DRIVES; i++) {
    ...
    // Cleanup
    std::cout << "[Info] Exiting fuse_session_loop\n";
    ...
    // Stop threads
    std::cout << "[Info] Signaling worker threads to stop\n";
