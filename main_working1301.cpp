...
// -----------------------------------------------------------------------------
// To fix the “static assertion failed: result type must be constructible”
// we provide explicit move constructors for structs that contain atomics.
struct LatencyStats {
...
struct ThreadStats {
    ...
    ThreadStats(ThreadStats&& other) noexcept {
        ...
        last_activity = other.last_activity; // normal copy
    }
...
public:
    ...
    int access_mask;
    std::promise<void> completion_promise; // optional
...
// One big lock for demonstration (instead of per-inode)
static std::mutex g_inodeLock;
class InodeGlobalLock {
public:
    InodeGlobalLock() { g_inodeLock.lock(); }
    ~InodeGlobalLock() { g_inodeLock.unlock(); }
};

// -----------------------------------------------------------------------------
// Declarations of processX
...
static void print_distribution_stats()
{
    ...
    for(int i=0; i<NUM_DRIVES; i++) {
        ...
        g_stats_file 
          ...
          << "  Latency: avg=" << avg_lat << "us, max=" << st.operation_latency.max_us.load() << "us\n"
...
// -----------------------------------------------------------------------------
// Implementation of processX calls
static void processLookup(FsRequest &r) {
    InodeGlobalLock guard;
