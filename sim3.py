#!/usr/bin/env python3
"""
Scalable Storage System Simulation with Realistic Workloads

This simulation models a storage system with multiple drives, fixed-length queues,
...
Features:
1. Configurable read/write latencies (realistic SSD/HDD behavior)
...
Usage:
    ...
    --duration 10 --rate 1000 --block-size 4096 --read-delay 0.01 --write-delay 0.05
    --queue-size 100
"""
...
BLOCK_SIZE = 4096
OPS_RATE = 1000          # Total operations per second
WORKLOAD_DURATION = 10   # Duration in seconds
READ_DELAY = 0.01        # Simulated processing delay for reads (seconds)
WRITE_DELAY = 0.05       # Simulated processing delay for writes (seconds)
...
class Drive:
    ...
    def __init__(self, drive_id, read_delay, write_delay, queue_size):
...
def init_drives(num_drives, read_delay, write_delay, queue_size):
    """Initialize drives dictionary with the given number of drives."""
    global drives, total_backpressure_events
    drives = {i: Drive(i, read_delay, write_delay, queue_size) for i in range(num_drives)}
    total_backpressure_events = 0
...
def run_workload_simulation(num_drives, duration, rate, block_size, read_delay, write_delay, 
    ...
    """
    global BLOCK_SIZE, OPS_RATE, WORKLOAD_DURATION, READ_DELAY, WRITE_DELAY, QUEUE_SIZE
    ...
    print(f"Read delay: {read_delay}s, Write delay: {write_delay}s, Queue size: {queue_size} ops")
    
    # Calculate theoretical maximum processing rates
    max_read_rate = 1.0 / read_delay if read_delay > 0 else float('inf')
    max_write_rate = 1.0 / write_delay if write_delay > 0 else float('inf')
    ...
    init_drives(num_drives, read_delay, write_delay, queue_size)
...
def main():
    ...
    parser.add_argument("--read-delay", type=float, default=0.01,
                        help="Simulated processing delay for reads (seconds)")
    parser.add_argument("--write-delay", type=float, default=0.05,
                        help="Simulated processing delay for writes (seconds)")
