#!/usr/bin/env python3
"""
Scalable Storage System Simulation with Realistic SSD Workloads

This simulation models a storage system with multiple SSDs, fixed-length queues,
and support for various realistic workload patterns:
- Sequential reads
- Sequential writes
- Random reads
- Random writes
...
Features:
1. Configurable read/write latencies with realistic SSD behavior
2. Fixed-length queues with backpressure
3. Parallel processing of I/O requests
4. Detailed performance metrics and statistics
5. Visualization of results across different workload patterns

Usage:
    python3 realistic_storage_simulation.py --workloads all --drive-list 1,2,4,8
    --duration 10 --rate 1000 --block-size 4096 --queue-size 100 --ssd-type consumer
"""
...
# SSD Types with realistic latency profiles (in milliseconds, converted to seconds in the code)
SSD_PROFILES = {
    # Consumer NVMe SSD (e.g., Samsung 970 EVO Plus)
    "consumer": {
        "seq_read": 0.00015,  # 0.15ms for sequential read
        "seq_write": 0.00050, # 0.50ms for sequential write
        "rand_read": 0.00035, # 0.35ms for random read (4K)
        "rand_write": 0.00080 # 0.80ms for random write (4K)
    },
    # Enterprise NVMe SSD (e.g., Intel Optane P5800X)
    "enterprise": {
        "seq_read": 0.00010,  # 0.10ms for sequential read
        "seq_write": 0.00030, # 0.30ms for sequential write
        "rand_read": 0.00020, # 0.20ms for random read (4K)
        "rand_write": 0.00040 # 0.40ms for random write (4K)
    },
    # Budget SATA SSD (e.g., Crucial MX500)
    "budget": {
        "seq_read": 0.00035,  # 0.35ms for sequential read
        "seq_write": 0.00100, # 1.00ms for sequential write
        "rand_read": 0.00080, # 0.80ms for random read (4K)
        "rand_write": 0.00180 # 1.80ms for random write (4K)
    }
}
...
SSD_TYPE = "enterprise"    # Default SSD type
...
class Operation:
    ...
    def __init__(self, op_id, op_type, offset, access_pattern=AccessPattern.SEQUENTIAL, size=None):
        self.op_id = op_id
...
class Drive:
    """Simulated SSD that processes read/write operations from its queue."""
    def __init__(self, drive_id, ssd_profile, queue_size):
        self.drive_id = drive_id
        self.ssd_profile = ssd_profile
    ...
    def get_operation_latency(self, operation):
        """Determine the appropriate latency based on operation type and access pattern"""
        if operation.op_type == OpType.READ:
            return self.ssd_profile["seq_read" if operation.access_pattern == AccessPattern.SEQUENTIAL else "rand_read"]
        else:  # WRITE
            return self.ssd_profile["seq_write" if operation.access_pattern == AccessPattern.SEQUENTIAL else "rand_write"]
    ...
    def process_operation(self, op):
        """Process a read or write operation with appropriate SSD latency"""
        # Record start time for latency calculation
        start_time = time.time()
        
        # Get and apply the appropriate latency based on operation type and access pattern
        operation_latency = self.get_operation_latency(op)
        time.sleep(operation_latency)
...
def init_drives(num_drives, ssd_profile, queue_size):
    """Initialize drives dictionary with the given number of drives."""
    global drives, total_backpressure_events
    drives = {i: Drive(i, ssd_profile, queue_size) for i in range(num_drives)}
    total_backpressure_events = 0
...
class WorkloadPattern:
    ...
    def get_next_operation(self):
        ...
        # Set access pattern based on workload type
        access_pattern = AccessPattern.SEQUENTIAL if self.sequential else AccessPattern.RANDOM
        ...
        return Operation(op_id, op_type, offset, access_pattern)
...
def run_workload_simulation(num_drives, duration, rate, block_size, ssd_type, 
                           queue_size, workload_pattern_name):
    """
    Run the simulation for a specific workload pattern:
      - Initialize global parameters.
      - Initialize drives with fixed-size queues and proper SSD profile.
    ...
    """
    global BLOCK_SIZE, OPS_RATE, WORKLOAD_DURATION, SSD_TYPE, QUEUE_SIZE
    ...
    # Get the SSD profile
    if ssd_type not in SSD_PROFILES:
        print(f"Warning: Unknown SSD type '{ssd_type}', defaulting to 'consumer'")
        ssd_type = "consumer"
    
    ssd_profile = SSD_PROFILES[ssd_type]
    ...
    print(f"SSD Type: {ssd_type}")
    print(f"Sequential Read: {ssd_profile['seq_read']*1000:.2f}ms, "
          f"Sequential Write: {ssd_profile['seq_write']*1000:.2f}ms")
    print(f"Random Read: {ssd_profile['rand_read']*1000:.2f}ms, "
          f"Random Write: {ssd_profile['rand_write']*1000:.2f}ms")
    print(f"Queue size: {queue_size} ops")
    
    # Calculate theoretical maximum processing rates for the SSD type
    if workload_pattern.sequential:
        max_read_rate = 1.0 / ssd_profile['seq_read'] if ssd_profile['seq_read'] > 0 else float('inf')
        max_write_rate = 1.0 / ssd_profile['seq_write'] if ssd_profile['seq_write'] > 0 else float('inf')
    else:
        max_read_rate = 1.0 / ssd_profile['rand_read'] if ssd_profile['rand_read'] > 0 else float('inf')
        max_write_rate = 1.0 / ssd_profile['rand_write'] if ssd_profile['rand_write'] > 0 else float('inf')
    ...
    effective_max_rate = (read_ratio * max_read_rate + (1 - read_ratio) * max_write_rate) * num_drives
    
    print(f"Theoretical max read rate: {max_read_rate:.1f} ops/s per drive")
    print(f"Theoretical max write rate: {max_write_rate:.1f} ops/s per drive")
    ...
    init_drives(num_drives, ssd_profile, queue_size)
...
def main():
    parser = argparse.ArgumentParser(description="Realistic Storage System Simulation with Multiple Workloads")
    ...
    parser.add_argument("--ssd-type", type=str, default="consumer", choices=["consumer", "enterprise", "budget"],
                        help="SSD type profile to use (consumer, enterprise, or budget)")
    ...
    print(f"Testing with {len(drive_list)} drive configurations: {drive_list}")
    print(f"Using SSD type: {args.ssd_type}")
    ...
    plot_workload_comparison(results, drive_list, workloads)
